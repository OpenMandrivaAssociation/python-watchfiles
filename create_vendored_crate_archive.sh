...
if [ -f $ARCHIVE_NAME ]; then
	ARCHIVE_NAME="$PACKAGE_NAME-$VSN.$PACKAGE_ARCHIVE_KIND"
...
if [ $SUBPKG_NAME ]; then
	cd ./${ARCHIVE_NAME%%.$PACKAGE_ARCHIVE_KIND}/$SUBPKG_NAME
else
...
if [ $SUBPKG_NAME ]; then
	...
	mv -f ./vendor.tar.xz "../$PACKAGE_NAME-$VERSION-vendor.tar.xz"
	cd ..
	printf "Moving vendor archive out to package directory at: $(pwd)/$PACKAGE_NAME-$VERSION-vendor.tar.xz\n"
fi
################################################################################
